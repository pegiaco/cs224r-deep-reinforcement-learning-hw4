...
from infrastructure import schedule, replay, rl_utils
from embed import embedders, encoder_decoder
    ...
    def from_config(cls, config, env):
        def embedder_factory():
            ...
            if embed_type == "instruction":
                return encoder_decoder.ExploitationPolicyEmbedder.from_config(
                        embedder_config, env)
            elif embed_type == "recurrent":
                return embedders.RecurrentStateEmbedder.from_config(
                        embedder_config, env)
            else:
                raise ValueError(
...
class RecurrentDQNPolicy(DQNPolicy):
    ...
    def loss(self, experiences, weights):
        ...
        experiences, mask = rl_utils.pad(experiences)
